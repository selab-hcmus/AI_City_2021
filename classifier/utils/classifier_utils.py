...
def train_model(model, logger, dataloaders, criterion, optimizer, scheduler, num_epochs=25, save_path=None):
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            logger.info(f"[{phase}] Loss: {epoch_loss:.4f}, Acc: {trackers[f'{phase}_acc'].avg:.4f}, Iou: {trackers[f'{phase}_iou'].avg:.4f}, Dice: {trackers[f'{phase}_dice'].avg:.4f}, Lr: {optimizer.param_groups[0]['lr']}")
            if phase == 'val':
                if epoch_acc > best_acc:
                    ...
                    if save_path is not None:
                        ...
                        logger.info(f"save best model at {epoch}")
                    else:
                        step_count += 1
